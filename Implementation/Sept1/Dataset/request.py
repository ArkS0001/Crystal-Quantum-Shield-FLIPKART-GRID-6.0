...
csv_file = 'log.csv'     # CSV file to log request details

# Initialize CSV file
def init_csv():
    with open(csv_file, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Request Number', 'Status Code', 'Rate Limit Exceeded', 'Proxy Jump'])

# Log request details to CSV
def log_to_csv(request_number, status_code, exceeded, proxy_jump):
    with open(csv_file, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([request_number, status_code, exceeded, proxy_jump])
...
def send_request(request_number):
    try:
        ...
        proxy_jump = response.headers.get('X-Proxy-Jump', 'None')
        log_to_csv(request_number, status_code, exceeded, proxy_jump)
        print(f"Request {request_number}: Status Code: {status_code}, Rate Limit Exceeded: {exceeded}, Proxy Jump: {proxy_jump}")
    except requests.RequestException as e:
