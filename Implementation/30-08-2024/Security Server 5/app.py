...
#         # Call the security server to verify login
#         security_response = requests.post(f"{SECURITY_SERVER_URL}/api/security/login", json={'email': email, 'password': password})
...
#             return jsonify(token=str(access_token)), 201
#         else:
#             return jsonify(message='Security verification failed'), 401
...
# def updateProduct():
#     pid = request.json.get('_id')
#     updated_product = request.json.get('product')
#     if pid in products_db:
#         products_db[pid].update(updated_product)
...
def adminRegister():
    ...
    # Call security server for registration verification
    security_response = requests.post(f"{SECURITY_SERVER_URL}/api/security/register", json={'email': email, 'password': password})
    if security_response.status_code != 201:
        return jsonify(message='Security verification failed'), 401
...
#     admin = admins_db.get(email)
#     if admin and bcrypt.checkpw(password.encode('utf-8'), admin['password'].encode('utf-8')):
#         # Call security server to verify login
#         security_response = requests.post(f"{SECURITY_SERVER_URL}/api/security/login", json={'email': email, 'password': password})
#         if security_response.status_code != 200:
#             return jsonify(message='Security verification failed'), 401
...
def adminLogin():
    ...
    # Try to decode and check the password hash
    try:
            # Call security server to verify login
        security_response = requests.post(f"{SECURITY_SERVER_URL}/api/security/login", json={'email': email, 'password': password})
        if security_response.status_code != 200:
        ...
        # Handle invalid bcrypt salt or other bcrypt errors
        return jsonify(message=f"Password verification failed: {str(e)}"), 500
...
def getAdminData():
    ...
    for admin_id, admin in admins_db.items():
        # Use .get('tokens', []) to safely handle missing 'tokens' key
        tokens = admin.get('tokens', [])
        if any(token['token'] == auth_token for token in tokens):
            admin_data = admin.copy()
            admin_data.pop('tokens', None)  # Exclude tokens from response
            return jsonify(admin_data), 200
